...
const Chatbox = () => {
  ...
  const serverEndpoint = 'https://example.com/chat';
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      const response = await axios.post(serverEndpoint, { message: inputMessage });

      // Update local state with the sent message
      setMessages((prevMessages) => [...prevMessages, { text: inputMessage, type: 'user' }]);

      // Receive and display the server's response
      setMessages((prevMessages) => [...prevMessages, { text: response.data.message, type: 'server' }]);
  ...
  return (
    <Container maxWidth="sm" sx={{ marginTop: '20px', textAlign: 'center' }}>
      <Paper elevation={3} sx={{ padding: '20px', minHeight: '300px', overflowY: 'auto' }}>
